                ...
                <div class="lg:col-span-2 space-y-5">
                    ...
                    <div class="bg-white rounded-xl border border-gray-100 shadow-[0px_1px_3px_rgba(0,0,0,0.05)] p-6"
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            ...
                            <button type="button" @click="current = {{ $i }}; lightbox = true"
                               class="relative aspect-square rounded-xl overflow-hidden border border-gray-100 hover:border-brand-green transition-colors group">
                                <img src="{{ Storage::url($foto->path) }}" alt="Foto avería"
                        ...
                        <div x-show="lightbox" x-cloak @keydown.escape.window="lightbox = false"
                            ...
                            <img :src="fotos[current]" alt="Foto avería" class="max-h-[85vh] max-w-[90vw] rounded-xl object-contain select-none">
                ...
                <div class="lg:col-span-1 space-y-5">
                    ...
                    <div class="bg-white rounded-xl border border-gray-100 shadow-[0px_1px_3px_rgba(0,0,0,0.05)] p-5"
                        ...
                        <button type="button" @click="open = true" class="block w-full">
                            <img src="{{ Storage::url($orden->firma_path) }}" alt="Firma"
                                 class="w-full rounded-lg border border-gray-100 cursor-zoom-in hover:opacity-80 transition-opacity">
                            <p class="text-xs text-gray-400 mt-2 text-center">Toca para ampliar</p>
                        </button>
                        <!-- Lightbox firma -->
                        <div x-show="open" x-cloak @keydown.escape.window="open = false"
                             class="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" @click.self="open = false">
                            <button @click="open = false"
                                class="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                            </button>
                            <img src="{{ Storage::url($orden->firma_path) }}" alt="Firma"
                                 class="max-h-[85vh] max-w-[90vw] rounded-xl bg-white p-4 object-contain">
                        </div>
                    </div>
                    @endif
